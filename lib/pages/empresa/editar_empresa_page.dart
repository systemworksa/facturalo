...
class _EditarEmpresaPageState extends State<EditarEmpresaPage> {
 ...
 // final Connectivity _connectivity = Connectivity();
  ...
  List<DropdownMenuItem<String>> getContribuyente() {
    ...
    listaCiudad.add(const DropdownMenuItem(
      child: SizedBox(
        width: 220.0,
        child: Text(
          'CONTRIBUYENTE RÉGIMEN RIMPE EMPRENDEDOR JNDA',
          style: TextStyle(fontSize: 11),
        ),
      ),
      value: 'CONTRIBUYENTE RÉGIMEN RIMPE EMPRENDEDOR JNDA',
    ));
    listaCiudad.add(const DropdownMenuItem(
      child: SizedBox(
        width: 220.0,
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          'A continuación mostramos los datos del RUC.',
                    ...
                    /*const SizedBox(height: 10),
                    MyCustomInputBox(
                      label: 'Valor del Impuesto (IVA)',
                      inputHint: 'Ingrese el impuesto...',
                      inputController: impuestoController,
                      textInputType: TextInputType.number,
                      formatter:true,                      
                    ),*/
                    const SizedBox(height: 10),
  ...
  guardarEmpresa() async {
    ...
    if (impuestoController.text == '') {
      message.showToast('El valor del impuesto es un campo obligatorio');
      return;
    }
  ...
  getEmpresa() async {
    ...
    resp.then((id) {
      ...
      for (var child in json2) {
        setState(() {
          _isLoading = false;
          cedulaController.text = child["informacionEmpresa"]["ruc"];
          razonSocialController.text =
              child["informacionEmpresa"]["razonSocial"];
          nombreComercialController.text =
              child["informacionEmpresa"]["nombreComercial"];
          ciudadController.text = child["informacionEmpresa"]["ciudad"];
          direccionController.text = child["informacionEmpresa"]["direccion"];
          celularController.text = child["informacionEmpresa"]["contacto"];
          emailController.text = child["informacionEmpresa"]["email"];
          codigoArtesanalController.text =
              child["informacionEmpresa"]["codigoArtesanal"];

          opcionObligado = child["informacionEmpresa"]["llevaContabilidad"];
          opcionObligado = opcionObligado.toUpperCase();
          opcionTipoContribuyente =
