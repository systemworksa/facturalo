...
class ValidarEmailPingPage extends StatefulWidget {
  const ValidarEmailPingPage({Key? key}) : super(key: key);

  @override
  State<ValidarEmailPingPage> createState() => _ValidarEmailPingPageState();
}

class _ValidarEmailPingPageState extends State<ValidarEmailPingPage> {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        ...
                        child: const Center(
                          child: Text(
                            '¿Desea iniciar con otro método? clic aquí',
                            style: TextStyle(
  ...
  iniciarSession() async {
    ...
    Future<dynamic> resp = usuarios.validarEmail(emailController.text);
    ...
    resp.then((id) {
      ...
      for (var child in json2) {
        if (child["status"]) {
          setState(() {
            _isLoading = false;
            prefs.setString('emailRecuperacion', emailController.text);
            Navigator.pushNamed(context, 'validar-codigo-ping');
          });
        } else {
