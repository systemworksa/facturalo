...
class CrearPingPage extends StatefulWidget {
  const CrearPingPage({Key? key}) : super(key: key);

  @override
  State<CrearPingPage> createState() => _CrearPingPageState();
}

class _CrearPingPageState extends State<CrearPingPage> {
 ...
 // final Connectivity _connectivity = Connectivity();
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Estimado usuario aún no ha creado su pin. Ingrese un pin de 4 digitos para acceder rapidamente en nuevas ingresos.',
  ...
  iniciarSession() async {
    if (pinController.text.toString() == "") {
      message.showToast('El pin es un campo obligatorio');
      return;
    }
    if (repetirPingController.text.toString() == "") {
      message.showToast('Por favor repita el pin que ingresó.');
      return;
    }
    if (repetirPingController.text.toString() !=
        pinController.text.toString()) {
      message.showToast(
          'El pin que repitió no coincide con el que ingreso previamente.');
      return;
    }
    setState(() {
      _isLoading = true;
    ...
    Future<dynamic> resp = usuarios.crearPing(prefs.getString('idUsuario'),
        prefs.getString('token'), pinController.text);
