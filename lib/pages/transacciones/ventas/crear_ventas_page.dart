...
class CrearVentaPage extends StatefulWidget {
  const CrearVentaPage({Key? key}) : super(key: key);

  @override
  State<CrearVentaPage> createState() => _CrearVentaPageState();
}

class _CrearVentaPageState extends State<CrearVentaPage>
    with TickerProviderStateMixin {
  final establecimientos = EstablecimientoProvider();
  List<Establecimiento> establecimientosItem = [];
  List<Establecimiento> buscadorEstablecimientosItem = [];

  final empresa = EmpresaProvider();
  String tarifaIva = '12';
  // ignore: unused_field
  bool _isLoadingTarifaIva = false;
 ...
 // final Connectivity _connectivity = Connectivity();
  ...
  var linkImg = 'assets/img/checkoff.png';
  ...
  String imagenEstablecimiento =
      "https://facturalo.com.ec/sistema/vistas/img/usuarios/default/empresa.png";
  String datosTransportista = "";
  bool cambiarSucursal = false;
  ...
  getConfig() async {
    ...
    _inputFieldObservacionController.text = prefs.getString('transportista')!;
  }
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          ' GUARDAR',
                          style:
                              TextStyle(color: colorBtnGuardar, fontSize: 11.0),
                        ),
                      ],
                    ),
                  ),
                                ...
                                Flexible(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        nombreEstablecimiento.length > 77
  ...
  getEmpresa() async {
    ...
    resp.then((id) {
      ...
      for (var child in json2) {
        setState(() {
          _isLoadingTarifaIva = true;
          tarifaIva = child["informacionEmpresa"]["tarifaIva"].toString();
        });
      }
    });
    setState(() {});
                                        ...
                                        onPressed: () {
                                          setState(() {
                                            ...
                                            if (contador[index] == 1) {
                                              ...
                                              array.add({
                                                "id":
                                                    band.idProducto.toString(),
                                                "idProducto":
                                                    band.idProducto.toString(),
                                                "idBodega":
                                                    opcionCiudad.split(",")[0],
                                                "stock": "",
                                                "nombre": band.nombre,
                                                "codigo": band.codigo,
                                                "observacion": "",
                                                ...
                                                "COD_PRO": band.id,
                                                "NOM_PRO": band.nombre,
                                                "PREC_PUB_PRO": band.precio,
                                                "DESC_PRO": band.descripcion,
                                                "DESCVAL_PRO": "0.00",
                                                "IVA_PRO": band.impuesto,
                                                "TOT_PRO": band.precio,
                                                "CANT_PRO":
                                                    _controllerSD[index].text,
                                                "FK_COD_SUC":
                                                    opcionCiudad.split(",")[0],
                                                "PREC_DISTTRIB_PRO":
                                                    band.precioDistribuidor,
                                                "PREC_MAY_PRO":
                                                    band.precioMayorista,
                                                "PREC_LIQ_PRO":
                                                    band.precioLiquidacion,
                                                "eliminar": true,
                                                "idCombo": "",
                                                "bloquear": "no",
                                                "preciomanual": band.pvpManual,
                                                "tipoIce":band.tipoIce,//NUEVO DESARROLLO ICE
                                                "codigoIce":band.codigoIce,//NUEVO DESARROLLO ICE
                                                "valorIce":band.valorIce,//NUEVO DESARROLLO ICE
                                              });*/

                                              //JSON NUEVO - ACTUALIZACION (MES JULIO 2023)
                                                  ...
                                                  if (contador[index] == 1) {
                                                    ...
                                                    array.add({
                                                      ...
                                                      "CANT_PRO":
                                                          _controllerSD[index]
                                                              .text,
                                                      "FK_COD_SUC": opcionCiudad
                                                          .split(",")[0],
                                                      "PREC_DISTTRIB_PRO": band
                                                          .paddrecioDistribuidor,
                                                      "PREC_MAY_PRO":
                                                          band.precioMayorista,
                                                      "PREC_LIQ_PRO": band
                                                          .precioLiquidacion,
                                                    ...
                                                    //JSON NUEVA ACTUALIZACION (MES JULIO 2023)
                                                    array.add({
                                                      "bodega": "CENTRAL",
                                                      "cantidad":
                                                          _controllerSD[index]
  ...
  calcularTotales() {
    ...
    double impuesto_baseIva = 0;
    for (var element in detalle) {
      total += double.parse(element.precio!);
      //DESARROLLO ICE
      if (element.tipoice != "No aplica") {
        if (element.tipoice == "Por porcentaje") {
          valorSubtotal = double.parse(element.cantidad.toString()) *
              double.parse(element.precio.toString());
          valorIce =
              valorSubtotal * (double.parse(element.valorIce.toString()) / 100);
          element.tarifaIce = valorIce.toString();
        } else if (element.tipoice == "Valor fijo") {
          valorIce = double.parse(element.cantidad.toString()) *
              double.parse(element.valorIce.toString());
          element.tarifaIce = valorIce.toString();
        }
      }
      ice += valorIce;
      ...
      if (int.tryParse(element.codigoimpuesto!.split(".")[0]) == 0) {
        //IVA CERO
        total0 += double.parse(element.total!);
      } else if (int.tryParse(element.impuesto!.split(".")[0]) != 0) {
        //IVA VARIABLE
        totalIva += double.parse(element.total!);
        impuesto_baseIva += (double.parse(element.total!) + valorIce) *
            ((double.parse(element.impuesto!)) / 100);
      } else if (int.tryParse(element.codigoimpuesto!.split(".")[0]) == 6) {
        //NO OBJETO
        totalNObjeto += double.parse(element.total!);
      } else if (int.tryParse(element.codigoimpuesto.toString()) == 7) {
        //EXCENTO
        totalExento += double.parse(element.total!);
      }
    }

    iva = impuesto_baseIva; //* ((double.parse(tarifaIva))/100);
              ...
              child: TextFormField(
                ...
                onChanged: (value) {
                  setState(() {
                    ...
                    double resp = double.parse(array[i]["cantidad"]) *
                        double.parse(array[i]["precio"]);

                    detalles.total = resp.toStringAsFixed(2);
                    detalle[i].total = resp.toStringAsFixed(2);
                    detalle[i].cantidad = array[i]["cantidad"] == ""
                        ? "0"
                        : array[i]["cantidad"].toString();
                    //detalle[i].cantidad = array[i]["cantidad"].toString()?? "0";
                    ...
                    if (array[i]["tipoice"] == "Por porcentaje") {
                      totalvalorIce =
                          resp * (double.parse(array[i]["valorice"]) / 100);
                    } else if (array[i]["tipoice"] == "Valor fijo") {
                      totalvalorIce = double.parse(detalle[i].cantidad == ""
                              ? "0"
                              : array[i]["cantidad"].toString()) *
                          double.parse(array[i]["valorice"]);
                    }
                  ...
                  setState(() {
                    ...
                    if (cajaTexto.contains("/") &&
                      ...
                      if (array[i]["tipoice"] == "Por porcentaje") {
                        ...
                        totalvalorIce = double.parse(array[i]["cantidad"]) *
                            double.parse(array[i]["valorice"]);
                      }
                      ...
                      detalle[i].precio = _controllerDT[i].text == ""
                          ? "0"
                          : _controllerDT[i].text;
  ...
  void guardarOrden() async {
    ...
    if (prefs.getString("idPuntoEmision") == null) {
      message.showToast('Asigne un Punto de Emision a la factura');
      setState(() {
    ...
    for (int i = 0; i < array.length; i++) {
      ...
      if (double.parse(array[i]["cantidad"]) <= 0) {
        message.showToast(
            'Debe ingresar una cantidad válida al producto ${array[i]["nombre"].toString()}');
        puedePresionarBoton = true;
        _isLoading = false;
        setState(() {
          colorBtnGuardar = Colors.white;
    ...
    Future<dynamic> resp = _documentos.guardarFactura(
        prefs.getString("token")!,
        prefs.getString("idUsuario")!,
        prefs.getString("idEmpresa")!,
        idCliente,
        prefs.getString("idPuntoEmision")!,
        opcionTipoVenta,
      ...
      for (var child in json2) {
        if (child["status"]) {
          ...
          if (opcionTipoVenta == 'Electronica') {
            _mostrarModalCorrecto('Factura realizada correctamente');
          } else {
          ...
          if (child["msg"] ==
              "Plan de facturacion consumido. Contacte con soporte para contratar un nuevo plan y seguir facturando.") {
            //ESTA OPCION DE PAGOS QUEDA PENDIENTE
        ...
        builder: (context) {
          return WillPopScope(
            ...
            child: AlertDialog(
              ...
              title: Column(
                children: [
                  SizedBox(child: Image.asset(linkImg), height: 120.0),
                  Center(
                    child: Column(
                      children: [
                        Text(
  ...
  onSearchTextChanged(String text, StateSetter setState) async {
    ...
    setState(() {
      productoItem.asMap().forEach((index, userDetail) {
        ...
        if (userDetail.codigo?.isEmpty ?? true) {
        } else {
          codigoVendedor = userDetail.codigo!;
        }
  ...
  Widget setupAlertEstablecimientos(StateSetter setState) {
    return SizedBox(
        height: 300.0,
        width: 300.0,
        child: buscadorEstablecimientosItem.isNotEmpty ||
                _inputFieldBuscarController.text.isNotEmpty
            ? ListView.builder(
                shrinkWrap: true,
                itemCount: buscadorEstablecimientosItem.length,
                itemBuilder: (context, i) => _bandListEstablecimientos(
                    buscadorEstablecimientosItem[i], i, setState),
              )
            : ListView.builder(
                shrinkWrap: true,
                itemCount: establecimientosItem.length,
                itemBuilder: (context, i) => _bandListEstablecimientos(
                    establecimientosItem[i], i, setState),
              ));
  }
  ...
  Widget _bandListEstablecimientos(
      Establecimiento emision, int index, StateSetter setState) {
    return GestureDetector(
      child: Container(
        padding: const EdgeInsets.all(2.0),
        child: Card(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                          ...
                          children: [
                            Text(
                                emision.nombre!.toString().length > 55
                                    ? emision.nombre!
                                            .toString()
                                            .substring(0, 55) +
                                        "..."
                                    : emision.nombre.toString(),
                                style: GoogleFonts.lato(
                                    textStyle: const TextStyle(
                                        color: Colors.black,
                                        fontSize: 13.0,
                            ...
                            /*
                            Text(emision.secuencialComprobante!.toString(),
                                      style: GoogleFonts.lato(
                                              textStyle: const TextStyle(
                                ...
                                child: Row(
                                  children: [
                                    ElevatedButton(
                                        onPressed: () {
                                          setState(() {
                                            seleccionarEstablecimiento(emision);
                                            Navigator.of(context).pop();
                                          });
                                        },
                                        style: ButtonStyle(
  ...
  void _mostrarModalEstablecimientos() {
    showDialog(
        context: context,
        barrierDismissible: false,
        //builder: (context) {
        ...
        builder: (BuildContext context) {
          return WillPopScope(
              onWillPop: () async {
                // Evita que el usuario cierre el diálogo presionando el botón de retroceso
                return false;
              },
              child: AlertDialog(
                shape: RoundedRectangleBorder(
                      ...
                      children: [
                        ...
                        Container(
                          child: TextField(
                            controller: _inputFieldBuscarController,
                            onChanged: (value) {
                              buscarEstablecimiento(value, setState);
                            },
                            decoration: ThemeHelper().textInputDecoration(
                                'Buscar', 'Buscar Punto de Emision'),
                          ),
                          decoration: ThemeHelper().inputBoxDecorationShaddow(),
                        ),
                        const SizedBox(height: 20.0),
                        Expanded(child: setupAlertEstablecimientos(setState)),
                      ],
                    ),
                  );
                }),
                actions: <Widget>[
                  TextButton(
                      onPressed: () {
                        if (!cambiarSucursal) {
                          message.showToast(
                              'Debe seleccionar un Punto de Emisión.');
                        } else {
                          Navigator.of(context).pop();
                        }
                      },
  ...
  getEstablecimientos() async {
    ...
    final data = await establecimientos.getEstablecimientos(
        prefs.getString('token'), prefs.getString('idEmpresa'), puntoemision);
    ...
    if (puntoemision == 0 || cambiarSucursal == true) {
      //(puntoemision==0)NO TIENE SELECCIONADO UN ESTABLECIMIENTO POR DEFECTO
      //(cambiarSucursal==true)CUANDO PRESIONAN EL BOTON "cambiar" EN PUNTO DE EMISION
      if (puntoemision == 0) {
        //Por debajo se asigna uno por defecto debido a que puede forzar cerrar la ventana
        establecimientosItem.forEach((element) {
          print(element);
          setState(() {
            prefs.setString('idPuntoEmision', element.id.toString());
            nombreEstablecimiento = element.nombre.toString();
            secuencialEstablecimiento =
                element.secuencialComprobante.toString();
            imagenEstablecimiento = element.imagen.toString();
            _inputFieldObservacionController.text =
                element.datosTransportista.toString();
            cambiarSucursal = false;
          });
          return;
        });
        message.showToast('Seleccione un Punto de Emisión predeterminado.');
      }

      _mostrarModalEstablecimientos();
    } else {
      //ESTABLECIMIENTO SELECCIONADO

      bool finalizar = false;
      establecimientosItem.forEach((element) {
        if (element.id == puntoemision.toString()) {
          finalizar = true;
          setState(() {
            prefs.setString('idPuntoEmision', element.id.toString());
            nombreEstablecimiento = element.nombre.toString();
      ...
      //SI LLEGA AQUI SIGNIFICA QUE EL ESTABLECIMIENTO PREDETERMINADO FUE ELIMINADO O YA NO EXISTE
      _mostrarModalEstablecimientos();
    }
  }
  ...
  seleccionarEstablecimiento(valor) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      prefs.setString('idPuntoEmision', valor.id);
      nombreEstablecimiento = valor.nombre;
      secuencialEstablecimiento = valor.secuencialComprobante;
      imagenEstablecimiento = valor.imagen;
      _inputFieldObservacionController.text = valor.datosTransportista;
      cambiarSucursal = false;
    });
  }

  /* -------------------------------------------------------------------------- */
  /*                              BUSCADOS ESTABLECIMIENTOS                            */
  /* -------------------------------------------------------------------------- */
  buscarEstablecimiento(String text, StateSetter setState) async {
    buscadorEstablecimientosItem.clear();
    ...
    setState(() {
      establecimientosItem.asMap().forEach((index, ptoEmision) {
        String codigo = ptoEmision.id?.toString() ?? '';
        String nombre = ptoEmision.nombre?.toString() ?? '';
        String direccion = ptoEmision.direccion?.toString() ?? '';
        String secuencialElectronico =
            ptoEmision.secuencialElectronico?.toString() ?? '';
        String secuencialFisico = ptoEmision.secuencialFisico?.toString() ?? '';
        String estado = ptoEmision.estado?.toString() ?? '';
        String limiteFactura = ptoEmision.limiteFactura?.toString() ?? '';
        ...
        if (codigo.toUpperCase().contains(text.toUpperCase()) ||
          ...
          buscadorEstablecimientosItem.add(ptoEmision);
        }
      });
    });
  }
